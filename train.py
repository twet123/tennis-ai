...
def train(env, actions, model, target_model):
    ...
    try:
        ...
        if os.path.isfile("epsilon.pckl"):
            with open("epsilon.pckl", "rb") as f:
                AgentParams.epsilon = pickle.load(f)
            print("usao", AgentParams.epsilon)
        ...
        while True:
            ...
            observation, reward, done, _, _ = env.step(1)  # fire on reset
            ...
            for timestep in range(1, AgentParams.max_steps_per_episode):
                ...
                if reward == -1 or reward == 1:
                    action = 1  # fire when point is starting
                ...
                if (
                    ...
                    state_next_sample = np.array(
                        [state_next_history[i] for i in indicies]
                    )
                    ...
                    updated_q_values = (
                        rewards_sample
                        + AgentParams.gamma * keras.ops.amax(future_rewards, axis=1)
                    )
            ...
            if (
                AgentParams.max_episodes > 0
                and episode_count >= AgentParams.max_episodes
